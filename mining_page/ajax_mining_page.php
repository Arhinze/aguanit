...
if((isset($_GET["un"])) && ((isset($_GET["up"])))){
    ...
    if ($data) {
        if ($data->mining_status != "active"){
            ...
            echo "<div class='pop_up'> Congrats, you've successfully initiated the mining cycle. <span style='float:right;position:absolute;top:6px;right:6px'><i class='fa fa-times' onclick='close_pop_up()'></i></span></div>";
        } else {
            echo "<div class='invalid'>Your mining cycle is already on. <span style='float:right;margin:4px 18px'><i class='fa fa-times' onclick='hide_invalid_div()'></i></span></div>";
        }
    } else {
